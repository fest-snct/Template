...
// OGP settings
$ogp_title = '出店一覧 | ' . $site_config['festival_label'];
$ogp_description = $site_config['festival_label'] . 'の出店一覧です。各クラスや部活動、教職員による出店情報を確認できます。';
    ...
    <main>
        ...
        <div id="s_container">
            <?php foreach ($stores as $store) : ?>
                <figure id="<?= htmlspecialchars($store['id'], ENT_QUOTES, 'UTF-8') ?>" 
                        class="s_items" 
                        data-name='<?= htmlspecialchars($store['name'], ENT_QUOTES, 'UTF-8') ?>'
                        data-location='<?= htmlspecialchars($store['location'], ENT_QUOTES, 'UTF-8') ?>'
                        data-description='<?= htmlspecialchars($store['description'], ENT_QUOTES, 'UTF-8') ?>'
                        data-news-link='<?= htmlspecialchars($store['news_link'] ?? '', ENT_QUOTES, 'UTF-8') ?>'
                        data-news-title='<?= htmlspecialchars($store['news_slug'] ?? '', ENT_QUOTES, 'UTF-8') ?>'
                        >
